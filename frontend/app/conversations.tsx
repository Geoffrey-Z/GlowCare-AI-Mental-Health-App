...
// Dynamic import and Audio namespace support
...
export default function ConversationAnalysisScreen() {
  ...
  const ensureIOSAudioMode = async () => {
    ...
    try {
      await setAudioModeAsync({ allowsRecordingIOS: true, playsInSilentModeIOS: true, interruptionModeIOS: 1, staysActiveInBackground: false });
      return true;
    } catch (e) {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      const modeOk = await ensureIOSAudioMode();
      if (!modeOk) {
        Alert.alert('Error', '无法启用 iOS 录音模式（AudioSession）。请重启设备或稍后重试。');
        return;
      }

      if (audioRecorder?.prepareToRecordAsync) await audioRecorder.prepareToRecordAsync();
      if (audioRecorder?.start) await audioRecorder.start();
      else if (audioRecorder?.record) audioRecorder.record();
      else throw new Error('Recorder has no start/record method');
            ...
            <View style={styles.resultsSection}>
              ...
              {analysisResult.analysis && (
                <View style={styles.analysisCard}>
                  ...
                  {analysisResult.analysis.triggers?.length > 0 && (
                    <View style={{ marginTop: 12 }}><Text style={styles.subTitle}>Triggers</Text><View style={styles.chipsWrap}>{analysisResult.analysis.triggers.map((t: string, i: number) => (<Chip key={`trig-${i}`} label={t} />))}</View></View>
                  )}
                  {analysisResult.analysis.distortions?.length > 0 && (
                    <View style={{ marginTop: 12 }}><Text style={styles.subTitle}>Cognitive Distortions</Text><View style={styles.chipsWrap}>{analysisResult.analysis.distortions.map((d: string, i: number) => (<Chip key={`dist-${i}`} label={d} tone="#fff7ed" textColor="#9a3412" />))}</View></View>
                  )}
                </View>
              )}
